 ...
 * A local storage row is the unique-key check. A pending rule row is not
 * emitted: the verifier blocks that case. The route picks the projection,
 * so that narrow stays on the controller. A duplicate slot in this store
 * is not a PostgreSQL atomic constraint.
 */
...
export async function emitBehavior(
  ...
  if (id === 'implement.accessScope') return done(emitAccess(definition, output));
  if (id === 'implement.authorityMap') return done(emitAuthority(definition, output));
  if (id === 'implement.repositoryPort') return memoryPort(definition, output);
...
function memoryPort(definition: M1Definition, output: string): EmitResult | EmitFailure {
  ...
  if (!listed.includes('await table().insert') || !listed.includes('table().findMany')) {
    return { code: 'STUB_SHAPE', detail: `${definition.artifactId} port methods were not recognized.` };
  }
  const source = listed.replace('export const pending', `${storeSource(entity)}\nexport const pending`);
...
async function grantPending(
  ...
  if ('code' in controller) return { pending: '', unread: true };
  ...
  if (!handler || grantIds.length === 0) return { pending: '', unread: true };
  const scopeDep = controller.dependencies.find(path => path.endsWith('/accessScope.defs.ts'));
  if (!scopeDep) return { pending: '', unread: true };
  const scope = await loadDefinition(scopeDep, read);
  if ('code' in scope) return { pending: '', unread: true };
  ...
  if (matched.length !== grantIds.length) return { pending: '', unread: true };
  const pending = matched.map(grant => text(grant.pending)).find(Boolean) ?? '';
  return { pending, unread: false };
}
