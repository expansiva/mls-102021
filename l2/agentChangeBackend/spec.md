...
**Repair loop + juiz (implementado 2026-07-04 — bloco compartilhado com o ns2;`improveAddNewSolution2_1.md` §4.3/§4.4):**
...
**O status escrito tem de ser o status persistido.** O `cb-finalize` relê o `todoBackend` e compara com o que o run acredita ter escrito, nas duas superfícies que podem virar o arquivo em disco (conteúdo do stor e modelo Monaco); divergência dá warning alto + um retry, e persistindo o step falha. Isso existe porque um run pode terminar verde sobre um arquivo com o estado PRÉ-run (petShop, 21/08/2026: relatório com 65 owners `done`, disco com 64 `toCreate`), e o custo do silêncio é o run seguinte regenerar do zero um módulo íntegro. Ver `flow.json → conventions.defsWritePersistence`.
