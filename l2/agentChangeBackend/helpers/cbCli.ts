...
const CLI_KEYWORDS = new Set(['rebuild', 'all', 'defs', 'seeds', 'run', 'help', 'fast', 'nochain']);
...
export function parseCli(raw: string | undefined): { kind: CbCommandKind; module: string; noAssets: boolean; fast: boolean; nochain: boolean } {
  ...
  const nochain = lower.some(t => t === 'nochain');
  let module = '';
  ...
  return { kind, module, noAssets, fast, nochain };
}
