/// <mls fileReference="_102021_/l2/agentChangeBackend/steps/gen-port/agentCbRepositoryPort.test.ts" enhancement="_blank"/>
...
void test('agentCbRepositoryPort declares the LLM step agent contract', () => {
  const src = readFileSync(path.join(HERE, 'agentCbRepositoryPort.ts'), 'utf8');
...
void test('agentCbRepositoryPort tool schema is provider-clean', () => {
  const errs = lintToolSchema(JSON.stringify(tool().function.parameters));
  assert.equal(errs, null, errs?.join(' | '));
});

for (const modelType of MODEL_TYPES) {
  void test(`agentCbRepositoryPort live @ ${modelType}: schema accepted + result has items`, { skip: !liveTestsEnabled() }, async () => {
    const r = await callToolProvider(config(), { modelType, system: system(modelType), human: human({ items: [{ entityId: 'Order', interfaceName: 'IOrderRepository', methods: [] }] }), tool: tool() });
...
function tool(): any {
  return createPlannerToolSchema(TOOL_NAME, 'Submit the repository port interfaces.', batchSchema(repositoryPortResultSchema as Record<string, unknown>));
}
